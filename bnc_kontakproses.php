...
if(strtoupper($_POST['kode']) == $_SESSION['captcha_id'])
{
	...
	if (isset($error)) {
		...
		{
			$subjek = "BNC Kontak - ".$_POST['subject'];
		}


		$h = $kontrol->inputKontak($_POST['name'], $_POST['email'], $subjek, $_POST['message'], '2', '-');
		if($h <= 0)
		{
			echo '<div><br/></div>
			<div class="col-md-12 col-sm-6">
...
?>
			...
			<div class="col-md-12 col-sm-6">
			    <div class="service-block service-block-sea service-or">
			        ...
			        <h2 class="heading-md">Pesan Anda telah disimpan.</h2>
			        <p>Customer service kami akan merespon pesan Anda secepatnya.</p>
			        <p>Terima kasih.</p>                        
			    </div>
			</div>
<?php
